    ...
    int       nCmdShow)
{
    ...
    return Flappy_birdPlayer::createAndRun();
}

int Flappy_birdPlayer::createAndRun(void)
{
    Flappy_birdPlayer *host = Flappy_birdPlayer::sharedInstance();
    int ret = host->run();
    Flappy_birdPlayer::purgeSharedInstance();
    return ret;
}

Flappy_birdPlayer *Flappy_birdPlayer::s_sharedInstance = NULL;

Flappy_birdPlayer *Flappy_birdPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new Flappy_birdPlayer();
    }
    return s_sharedInstance;
}

void Flappy_birdPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
Flappy_birdPlayer::Flappy_birdPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int Flappy_birdPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "flappy_bird-player");
...
void Flappy_birdPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void Flappy_birdPlayer::createViewMenu(void)
{
...
void Flappy_birdPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void Flappy_birdPlayer::updateWindowTitle(void)
{
}

void Flappy_birdPlayer::relaunch(void)
{
...
void Flappy_birdPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void Flappy_birdPlayer::onFileOpenProject(void)
{
...
void Flappy_birdPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"Flappy_birdPlayer");
...
void Flappy_birdPlayer::onFileProjectConfig(void)
{
...
void Flappy_birdPlayer::onFileRelaunch(void)
{
    relaunch();
}

void Flappy_birdPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void Flappy_birdPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void Flappy_birdPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void Flappy_birdPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void Flappy_birdPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT Flappy_birdPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    Flappy_birdPlayer *host = Flappy_birdPlayer::sharedInstance();
...
INT_PTR CALLBACK Flappy_birdPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
